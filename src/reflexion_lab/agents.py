...
class BaseAgent:
    ...
    def run(self, example: QAExample) -> RunRecord:
        ...
        for attempt_id in range(1, self.max_attempts + 1):
            ...
            reflector_tokens = 0
            if self.agent_type == "reflexion" and attempt_id < self.max_attempts and judge.score == 0:
                reflection, reflector_tokens = reflector(example, attempt_id, judge, answer)
                reflections.append(reflection)
                reflection_memory.append(f"Attempt {attempt_id} failed. Reason: {judge.reason}\nLesson: {reflection.lesson}\nStrategy: {reflection.next_strategy}")

            token_estimate = actor_tokens + judge_tokens + reflector_tokens
